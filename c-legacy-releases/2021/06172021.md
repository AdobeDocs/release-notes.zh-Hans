...
| 功能 | 描述 |
| ------- | ----------- |
| [!UICONTROL 促销活动] | (Google广告促销活动) [!UICONTROL Advertising Cloud Search] 现在，可以同步、优化和提供购物促销活动中本地库存广告的归因。<br><br>在Google广告中设置和管理本地库存广告。 |
...
| [!UICONTROL 通知中心 Beta 版] | 您现在可以通过安装 [!UICONTROL 通知中心测试版] 适用于Google Chrome 40及更高版本或Microsoft Edge 17及更高版本的应用程序。 应用程序加载速度比常规浏览器版本快，并会自动更新。<br><br>安装应用程序后，Web应用程序将作为一个单独的窗口加载，该窗口会根据窗口大小动态重新排列其布局。 您无需登录应用程序，也无需打开浏览器窗口即可使用它。 |
|  | 新警报类型， [!UICONTROL Campaign Management UI操作]，可用并显示所有后台作业的最终状态，包括对组合、约束、分类和事件的批量编辑和分配更改。 您自动订阅了UI操作警报。<br><br>此警报类型最终将替换右侧工具栏中提供的旧作业状态通知（旧作业状态通知）。 |
| 报表 | 新 [!UICONTROL 促销活动每小时报表] 显示一个或多个Google促销活动的成本、点击和（可选）转化数据，其中包含指定日期范围内每小时的关键词。 此报表与 [!UICONTROL 促销活动报表] 数据聚合为“每小时”或“每周日期（每小时）”。 这些聚合类型将从 [!UICONTROL 促销活动报表] 在未来版本中。 |
