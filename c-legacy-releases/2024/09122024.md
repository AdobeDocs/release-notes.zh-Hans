...
| 2 年 10 月 | 可自定义的主页 | 在 Experience Cloud 登陆页面上，点击&#x200B;**[!UICONTROL 编辑]**。[!UICONTROL 编辑]模式使您能够通过访问小组件库和自定义背景来个性化您的 Experience Cloud 主页。[!UICONTROL 编辑]模式为移动、调整大小和管理构件提供无缝、直观的控件，包括批量操作和布局调整，从而提供更美观且量身定制的体验。通过访问构件库和自定义背景，实现您的Experience Cloud主页个性化。 |
