...
| 2023 年 1 月 | [使用过滤器（标记）](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/analysis-workspace/navigating-workspace-projects/using-filters-tags-in-the-left-rail.html) | 更新的视频 | 了解如何在Analysis Workspace中使用名为标记的过滤器。 这些是帮助您高效构建项目的省时技术。 |
...
| 2023 年 1 月 | [使用键盘快捷键](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/analysis-workspace/navigating-workspace-projects/keyboard-shortcuts.html) | 更新的视频 | 了解如何使用Analysis Workspace中的键盘快捷键。 这些是帮助您在项目中执行各种任务的省时技术。 |
| 2023 年 1 月 | [复制和插入面板和可视化图表](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/analysis-workspace/navigating-workspace-projects/copy-insert-analysis-workspace.html) | 更新的视频 | 了解如何在Analysis Workspace中复制和插入面板和可视化图表。 这些是改进当前项目或在新项目上抢先一步的省时技巧。 |
...
| 发布日期 | 名称 | 类型 | 描述 |
| -----------| ---------- | ---------- | ---------- |
| 2023 年 1 月 | [使用 Report Builder 安排 Customer Journey Analytics 工作簿](https://experienceleague.adobe.com/docs/customer-journey-analytics-learn/tutorials/exporting/report-builder/schedule-cja-workbooks-using-report-builder.html) | 视频 | 了解如何在Report Builder中安排工作簿以进行Customer Journey Analytics。 通过安排工作簿，用户可自动执行使用最新数据刷新Report Builder工作簿并按照预定义的定期计划将其发送给利益相关者的过程。 |
...
* Experience Manager Assets现在为图像智能标记使用改进的人工智能框架。 这种内容智能可提高在引入时为所有图像资源提供的智能标记的相关性和准确性。此外，方向信息填充在`cq:tags`中，这样可以使用“方向”筛选器获得更好的搜索结果。
...
* [自适应表单向导](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/create-an-adaptive-form/create-an-adaptive-form-on-forms-cs/creating-adaptive-form.html)：Experience Manager Forms 提供便于企业用户使用的向导以快速创作自适应表单。该向导具有快速的选项卡导航，可轻松选择预配置的模板、样式、字段和提交选项以创建自适应表单。 此版本对向导进行了以下改进：
   * 选择或取消选择字段：可在向导中创建基于 JSON 和表单数据模型架构的自适应表单。现在可选择架构中的一部分字段以包括在自适应表单中。所选的字段被转换为相应的自适应表单数据捕获组件以快速创建所需的自定义表单。
   * 使用静态模板：在传统静态模板中有长期投入的客户可通过在模板中使用静态模板创建自适应表单，继续其采用云的历程。这为客户提供了额外的时间，以便将旧的静态模板迁移到现代的可编辑模板。
* [在服务器端处理时从记录文档(DoR)中删除隐藏字段](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/create-an-adaptive-form/generate-document-of-record-for-non-xfa-based-adaptive-forms.html)：您可以为最终用户生成记录文档PDF，其中仅包含他们在数据捕获体验期间可见的那些字段。 在提交表单时，服务器根据提交的数据验证对最终用户隐藏了哪些字段，并从记录文档中排除以达到一致。
...
* Dispatcher配置现在引用了一个文件，其中列出了常见的营销活动查询参数。 客户可以选择取消注释与其相关的参数，从而实现更好的缓存。 请参阅[营销活动参数](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/content-delivery/caching.html#marketing-parameters)，了解更多详细信息。
