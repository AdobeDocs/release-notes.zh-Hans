...
| 功能 | 描述 |
| ------- | ----------- |
| [!UICONTROL 项目组合]，[!UICONTROL 营销活动] | （选择启用测试版功能）一个新的[!UICONTROL 网络支出约束]选项，可让您为多网络项目组合中的特定广告网络定义预算分配限制。 该选项适用于具有每日、每周和每月支出策略的项目组合，并且仅在“[!UICONTROL 启用营销活动最大支出目标百分比]”选项未启用时可用。 选择“[!UICONTROL 自动调整营销活动预算限制]”选项后，约束将适用于为多个和最低营销活动预算设置配置的值。<br><br>基本报告中的新网络约束报告显示所有指定的最小和最大搜索引擎预算以及项目组合的实际支出（成本）。<br><br>要参加测试版计划，请联系您的客户团队。 |
